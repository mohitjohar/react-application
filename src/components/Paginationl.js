...
const Paginationl = ({data, maxitem, setFilterarr, maxbtn}) =>{
    ...
    const divider = (dl, max) =>{
        ...
        //filtered nav button
        const [fnav, setFnav] = useState(paginate(paginatepage,maxbtn,nxps))
        ...
        const Datashow = () =>{
            setFnav(paginate(paginatepage,maxbtn,nxps)) 
            setFilterarr(paginate(data,maxitem,nxps))
        }
        ...
        console.log("Next Prev Counter", nxps, " ", fnav)
         ...
         const pagenav = fnav && fnav.map((d, i)=>{
             if(d==active){
                return <li className="nav-item"><button className="btn btn-primary" onClick={()=>step1(d)}>{d}</button></li>
             }
             else{
